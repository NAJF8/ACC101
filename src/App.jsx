...
function MonthlyExpenses({ setToast }) {
  ...
  return <div className="screen-stack">
    <Panel title="مصاريف الشهر" action="سجل مالي واحد — طرق عرض متعددة">
      ...
      {busy ? <LoadingBlock /> : !monthRows.length ? <Empty text={viewMode === "month" ? "لا توجد حركات لهذا الشهر" : "لا توجد حركات ضمن الفترة"} /> : <><div className="report-summary-grid category-card-grid">{categoryRows.map((item) => <button type="button" className={`report-summary-card category-card ${activeCategory === item.name ? "active" : ""}`} key={item.name} onClick={() => setActiveCategory(activeCategory === item.name ? "" : item.name)}><span>{item.name}</span><strong>{money(item.total)}</strong><small>{item.count.toLocaleString("ar-IQ")} حركة</small></button>)}{activeCategory && <button type="button" className="secondary show-all-categories" onClick={() => setActiveCategory("")}>إظهار الكل</button>}</div>{viewMode === "month" && <Panel title={`مقارنة ${monthLabel(month)} مع ${monthLabel(comparisonMonth)}`} action="شهران فقط"><div className="comparison-grid">{[["إجمالي الخارج", total, comparison.cash_outflow], ["المصاريف", operating, comparison.operating_expenses], ["المشتريات", purchases, comparison.purchases], ["الرواتب", aggregation.payroll, comparison.payroll], ["الأصول", assets, comparison.assets]].map(([label, current, other]) => <div className="comparison-card" key={label}><span>{label}</span><strong>{money(current)}</strong><small>الفرق: {money(Math.abs(current - other))}</small></div>)}</div></Panel>}{filtered.length ? <DataTable rows={filtered.map((row) => ({ ...row, category_display: rowCategory(row), source_display: row.__source }))} columns={[["date", "التاريخ", (value, row) => value || row.purchase_date || row.transaction_date || "—"], ["description", "الوصف", (value, row) => value || row.name || row.item_name || row.reason || "—"], ["beneficiary", "الشخص / المورد", (value, row) => value || row.supplier_name || row.person_name || row.employee_name || "—"], ["amount_display", "المبلغ", money], ["category_display", "الفئة"], ["subcategory_name", "القسم الفرعي", (value) => value || "—"], ["payment_method", "طريقة الدفع", (value) => ({ cash: "نقدي", electronic: "إلكتروني", transfer: "تحويل", historical: "تاريخي" }[value] || value || "—")], ["accounting_class", "التصنيف المحاسبي"], ["source_display", "المصدر"], ["id", "رقم السجل"]]} /> : <Empty text="لا توجد حركات مطابقة للبحث أو الفلاتر" />}</>}
    </Panel>
  </div>;
}
